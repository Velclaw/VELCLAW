...
function secret() {
  if (!secretValue || secretValue.length < 32)
    throw new Error('VELCLAW_OAUTH_SECRET must contain at least 32 characters')
...
export function getOAuthClients(): OAuthClient[] {
  ...
  try {
    ...
    return parsed.filter(
      (c): c is OAuthClient =>
        c && typeof c.clientId === 'string' && typeof c.name === 'string' && Array.isArray(c.redirectUris),
    )
  } catch {
    return []
  }
...
export async function createAccessToken(input: { userId: string; clientId: string; scopes: string[] }) {
  return new SignJWT({ typ: 'access_token', client_id: input.clientId, scope: input.scopes.join(' ') })
    .setProtectedHeader({ alg: 'HS256' })
    .setIssuer(issuer)
    .setSubject(input.userId)
    .setIssuedAt()
