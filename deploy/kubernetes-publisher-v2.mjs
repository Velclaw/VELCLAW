...
const safe = (value, fallback = 'app') => {
  const result = String(value || fallback).toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '')
  return (result || fallback).slice(0, 50).replace(/-+$/g, '')
}
const appName = job => `vc-${safe(job.projectName)}-${safe(String(job.id).replace(/-/g, '').slice(0, 8))}`
const hostname = job => {
  const host = String(job.customDomain || `${safe(job.projectName)}-${safe(String(job.id).replace(/-/g, '').slice(0, 8))}.${DOMAIN}`).trim().toLowerCase()
  if (host !== DOMAIN && !host.endsWith(`.${DOMAIN}`)) throw new Error('Runtime hostname must remain inside the Velclaw product domain')
  return host
}
...
const pluralFor = { Secret: 'secrets', Job: 'jobs', Deployment: 'deployments', Service: 'services', Ingress: 'ingresses' }
function basePath(apiVersion, plural) {
  const parts = apiVersion.split('/')
  return parts.length === 1 ? `/api/${parts[0]}/namespaces/${NAMESPACE}/${plural}` : `/apis/${parts[0]}/${parts[1]}/namespaces/${NAMESPACE}/${plural}`
}
async function apply(resource) {
  const plural = pluralFor[resource.kind]
...
function buildJob(job) {
  ...
  return {
    ...
    spec: { backoffLimit: 0, ttlSecondsAfterFinished: 900, activeDeadlineSeconds: Math.ceil(JOB_TIMEOUT_MS / 1000), template: { metadata: { labels: { 'app.kubernetes.io/name': app, 'velclaw.deployment': job.id } }, spec: {
      ...
      containers: [{ name: 'kaniko', image: 'gcr.io/kaniko-project/executor:v1.23.2-debug', args: [`--context=dir:///workspace/src`, '--dockerfile=/workspace/src/Dockerfile', `--destination=${image}`, '--cache=true', '--cache-ttl=24h'], volumeMounts: [{ name: 'source', mountPath: '/workspace' }, { name: 'docker-config', mountPath: '/kaniko/.docker' }], securityContext: { runAsUser: 0, allowPrivilegeEscalation: false, capabilities: { drop: ['ALL'] } } }],
    } } },
  }
}
...
async function waitForJob(jobName) {
  ...
  while (Date.now() < deadline) {
    ...
    if ((status.failed || 0) > 0) throw new Error(`Kubernetes build job failed (failed=${status.failed})`)
    await new Promise(resolve => setTimeout(resolve, 3000))
  }
  throw new Error('Kubernetes build job timed out')
}

async function waitForDeployment(app) {
  const deadline = Date.now() + ROLLOUT_TIMEOUT_MS
  while (Date.now() < deadline) {
    const result = await k8s(`/apis/apps/v1/namespaces/${NAMESPACE}/deployments/${app}`)
    if (result.status === 404) throw new Error('Application Deployment disappeared')
    const desired = Number(result.data.spec?.replicas || 1)
    const status = result.data.status || {}
    if (Number(status.availableReplicas || 0) >= desired && Number(status.updatedReplicas || 0) >= desired && Number(status.unavailableReplicas || 0) === 0) return
    if ((status.conditions || []).some(c => c.type === 'Progressing' && c.reason === 'ProgressDeadlineExceeded')) throw new Error(`Rollout exceeded progress deadline: ${app}`)
    await new Promise(resolve => setTimeout(resolve, 3000))
  }
  throw new Error('Application rollout timed out')
}

function resources(job) {
  const app = appName(job), host = hostname(job), image = imageName(job)
  const env = Object.fromEntries(Object.entries(job.env || {}).filter(([key, value]) => /^[A-Za-z_][A-Za-z0-9_]{0,127}$/.test(key) && typeof value === 'string' && !/[\r\n]/.test(value)).map(([key, value]) => [key, value.slice(0, 8192)]))
  return [
...
async function publish(job) {
  ...
  try {
    if (job.operation === 'rollback' && job.rollbackTargetId) logs.push(`Rollback selected existing image from deployment ${job.rollbackTargetId}`)
      ...
      await waitForJob(build.metadata.name)
      logs.push('Build job completed and image pushed to registry')
    }
    const app = appName(job)
    for (const resource of resources(job)) await apply(resource)
    logs.push(`Applied Deployment/Service/Ingress for ${app}`)
    await waitForDeployment(app)
    logs.push(`Deployment ${app} passed readiness rollout verification`)
    await control(`/api/deployments/${job.id}/runtime`, { method: 'POST', body: JSON.stringify({ status: 'ready', url: `https://${hostname(job)}`, logs: logs.slice(-500) }) })
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    logs.push(`ERROR: ${message}`)
    await control(`/api/deployments/${job.id}/runtime`, { method: 'POST', body: JSON.stringify({ status: 'failed', logs: logs.slice(-500), error: message }) }).catch(() => {})
  }
}
