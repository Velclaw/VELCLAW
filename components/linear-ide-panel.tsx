...
export function LinearIdePanel({ defaultRepoUrl = 'https://github.com/Velclaw/VELCLAW' }: Props) {
  ...
  async function load() {
    ...
    setError('')
    try {
      const response = await fetch(teamId ? `/api/linear?teamId=${encodeURIComponent(teamId)}` : '/api/linear', {
        cache: 'no-store',
      })
    ...
    return (
      <aside className="w-full shrink-0 border-t border-white/10 bg-[#0b0d11] p-5 lg:w-[360px] lg:border-l lg:border-t-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold">Linear Build</div>
            <div className="text-xs text-white/40">Issue → IDE → Build</div>
          </div>
        </div>
        <p className="mt-5 text-sm leading-6 text-white/55">
          Connect Linear to turn project issues into executable Velclaw build tasks without leaving the IDE.
        </p>
        <a
          href="/api/linear/auth"
          className="mt-5 flex h-11 items-center justify-center gap-2 rounded-xl bg-white text-sm font-semibold text-black hover:bg-white/90"
        >
          <Link2 size={16} /> Connect Linear
        </a>
        {error && (
          <div className="mt-4 rounded-xl border border-red-400/20 bg-red-400/5 p-3 text-xs text-red-200">{error}</div>
        )}
      </aside>
    )
  }
    ...
    <aside className="flex w-full shrink-0 flex-col border-t border-white/10 bg-[#0b0d11] lg:w-[390px] lg:border-l lg:border-t-0">
      <div className="border-b border-white/10 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold">Linear Build</div>
              <div className="text-[11px] text-emerald-300/80">CONNECTED</div>
            </div>
          </div>
          <button
            onClick={() => void load()}
        ...
        <div className="mt-4 grid gap-2">
          <select
            ...
            className="h-10 rounded-lg border border-white/10 bg-black/20 px-3 text-xs text-white outline-none"
          >
            {teams.map((team) => (
          ...
          <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/20 px-3">
            ...
            <input
              ...
              className="h-10 min-w-0 flex-1 bg-transparent text-xs text-white outline-none"
      ...
      <div className="border-b border-white/10 p-4">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-[11px] font-semibold uppercase tracking-[.16em] text-white/35">New issue</span>
          <span className="text-[11px] text-white/25">{selectedTeam?.key || '—'}</span>
        </div>
        <div className="flex gap-2">
          <input
            ...
            className="h-10 min-w-0 flex-1 rounded-lg border border-white/10 bg-black/20 px-3 text-xs text-white outline-none"
            placeholder="What should Velclaw build?"
          />
          <button
            disabled={!title.trim() || !teamId || working === 'create'}
            onClick={() => void createIssue()}
            className="flex h-10 w-10 items-center justify-center rounded-lg bg-white text-black disabled:opacity-30"
          >
            <Plus size={16} />
          </button>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        ...
        <div className="space-y-2">
          {issues.map((issue) => (
            <div
              key={issue.id}
              className="rounded-xl border border-white/8 bg-white/[.025] p-3 transition hover:border-white/15"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="text-[10px] font-semibold text-white/35">
                  {issue.identifier} · {issue.state.name}
                </div>
                <a href={issue.url} target="_blank" rel="noreferrer" className="text-white/25 hover:text-white">
                  <ExternalLink size={13} />
                </a>
              </div>
              <div className="mt-1 text-sm font-medium text-white/90">{issue.title}</div>
              {issue.description && (
                <div className="mt-1 line-clamp-2 text-xs leading-5 text-white/35">{issue.description}</div>
              )}
              <button
                onClick={() => void buildIssue(issue)}
                disabled={working === issue.id}
                className="mt-3 flex h-9 w-full items-center justify-center gap-2 rounded-lg bg-white/[.07] text-xs font-semibold text-white hover:bg-white/[.12] disabled:opacity-50"
              >
                {working === issue.id ? <Loader2 size={14} className="animate-spin" /> : <Rocket size={14} />}{' '}
                {working === issue.id ? 'Starting build…' : 'Build in Velclaw'}
              </button>
            </div>
          ))}
      ...
      <div className="border-t border-white/10 p-3">
        <a
          href="/api/linear/auth"
          className="flex h-9 items-center justify-center gap-2 rounded-lg text-xs text-white/35 hover:bg-white/5 hover:text-white"
        >
          <Unplug size={14} /> Reconnect Linear
        </a>
      </div>
    </aside>
  )
