    ...
    <main className="flex-1 overflow-auto p-4 md:p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          ...
          <Card>
            <CardHeader className="p-4">
              ...
              <CardDescription>Release runtime của Velclaw.</CardDescription>
            </CardHeader>
            <CardContent className="flex items-center gap-2 px-4 pb-4 text-sm">
              <Container className="h-4 w-4" /> Docker + Traefik
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="p-4">
        ...
        <section className="space-y-3">
          <div>
            <h2 className="text-lg font-semibold">Canonical Velclaw pipeline</h2>
            <p className="text-xs text-muted-foreground">
              Deploy là delivery/evidence surface sau PR; không tạo pipeline Executor thứ hai.
            </p>
          </div>
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Đây là luồng deploy thật được thiết kế cho self-hosted Velclaw, không phụ thuộc platform hostname.
            </p>
          </div>
          <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-6">
            {runtimeFlow.map(([label, description], index) => (
              <div key={label} className="border border-border bg-card p-3">
                <div className="flex items-center gap-2">
                  {index === 3 ? (
                    <Container className="h-4 w-4 text-violet-300" />
                  ) : index === 4 ? (
          ...
          <div>
            <h2 className="text-lg font-semibold">Velclaw runtime delivery</h2>
            <p className="text-xs text-muted-foreground">
              Luồng deploy runtime của Velclaw: Docker build, container isolation và Traefik routing.
            </p>
          </div>
          <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-6">
            {runtimeFlow.map(([label, description], index) => (
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Mọi URL public do Velclaw công bố đều thuộc namespace của Velclaw.
            </p>
          </div>
        ...
        <section className="grid gap-3 md:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Runtime evidence</CardTitle>
              <CardDescription>Velclaw không tự nhận deployment thành công chỉ vì UI mở được.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-xs leading-5 text-muted-foreground">
              <p>
                Deployment evidence phải đến từ Velclaw runtime publisher/API: job được claim, Docker image build,
                container launch và URL first-party. Credentials vẫn nằm ngoài Skill metadata và UI.
              </p>
              <div className="flex flex-wrap gap-2">
