...
import { ArrowRight, CheckCircle2, Cloud, ExternalLink, Github, KeyRound, ListChecks, RefreshCw, ShieldCheck, Terminal } from 'lucide-react'
...
const targets = [
  ...
  { name: 'Velclaw Preview', host: '*.velclaw.cfd', state: 'preview', description: 'Velclaw-owned preview hostname. Platform-generated hostnames stay behind the deployment boundary.' },
  { name: 'Task Deployment', host: 'task.<branch>.velclaw.cfd', state: 'task-scoped', description: 'Deployment evidence associated with the Task and its release branch.' },
] as const
  ...
  return (
    <main className="flex-1 overflow-auto p-4 md:p-8">
      <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Deployment control plane riêng của Velclaw: chuẩn bị release, theo dõi evidence và chỉ công bố các URL thuộc namespace Velclaw.</p>
              </div>
            </div>
        ...
        <section className="space-y-3">
          <div><h2 className="text-lg font-semibold">Velclaw deployment targets</h2><p className="text-xs text-muted-foreground">Mọi URL hiển thị cho người dùng đều thuộc namespace Velclaw.</p></div>
        ...
        <section className="grid gap-3 md:grid-cols-2">
          <Card><CardHeader><CardTitle className="text-sm">Release checklist</CardTitle><CardDescription>Evidence cần có trước khi gọi một deployment thành công.</CardDescription></CardHeader><CardContent className="space-y-2 text-xs text-muted-foreground">{['Task có repository + branch hợp lệ', 'Skill selection và Executor hoàn tất', 'Review / UI Audit không còn blocker', 'Gate/CI checks có evidence', 'GitHub commit/PR đã được tạo', 'Deployment URL là hostname thuộc velclaw.cfd và đã được xác nhận'].map((item) => <div key={item} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-violet-300" />{item}</div>)}</CardContent></Card>
          <Card><CardHeader><CardTitle className="text-sm">Existing deployment evidence</CardTitle><CardDescription>Không tạo API giả nếu backend chưa hỗ trợ thao tác deploy trực tiếp.</CardDescription></CardHeader><CardContent className="space-y-3 text-xs leading-5 text-muted-foreground"><p>Task deployment API hiện có thể kiểm tra preview URL từ task, GitHub Checks, GitHub Deployments và commit statuses. Trang Velclaw Deploy đưa evidence đó vào cùng một UX và chỉ chấp nhận URL thuộc namespace Velclaw.</p><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" asChild><Link href="/tasks"><RefreshCw className="h-3.5 w-3.5" /> Task Deployments</Link></Button><Button variant="outline" size="sm" asChild><Link href="/api-keys"><KeyRound className="h-3.5 w-3.5" /> Deployment credentials</Link></Button></div></CardContent></Card>
        </section>

        <section className="border border-border bg-card p-4 text-xs leading-5 text-muted-foreground"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-violet-300">Boundary</p><p className="mt-2">Velclaw Deploy không tự nhận deployment đã thành công chỉ vì trang được mở. Production status chỉ được đánh dấu khi có evidence thật từ deployment checks. Platform-generated hostnames không được hiển thị như URL sản phẩm; URL public phải là <code>velclaw.cfd</code> hoặc một subdomain <code>*.velclaw.cfd</code>.</p></section>
