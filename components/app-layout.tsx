    ...
    >
      <div className="mb-3 md:mb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1">
            <span className="text-xs font-medium px-2 py-1 text-foreground bg-accent">Tasks</span>
            <span className="text-xs font-medium px-2 py-1 text-muted-foreground">Repos</span>
          </div>
          <div className="flex items-center gap-1">
      ...
      <div className="space-y-1">
        {Array.from({ length: 3 }).map((_, i) => (
          <Card key={i} className="animate-pulse h-[70px]">
...
export function AppLayout({ children, initialSidebarWidth, initialSidebarOpen, initialIsMobile }: AppLayoutProps) {
  ...
  useEffect(() => {
    if (!isStandalone) fetchTasks()
  }, [fetchTasks, isStandalone])
  useEffect(() => {
  ...
  useEffect(() => {
    const move = (e: MouseEvent) => {
      ...
      if (width >= 200 && width <= 600) updateSidebarWidth(width)
    }
    ...
    >
      <ConnectorsProvider>
        {isStandalone ? (
          ...
          >
            {isSidebarOpen && (
              <div className="lg:hidden fixed inset-0 bg-black/60 z-30" onClick={closeSidebar} aria-hidden="true" />
            )}
            <aside
              className={`fixed inset-y-0 left-0 z-40 ${isResizing || !hasMounted ? '' : 'transition-transform duration-200 ease-out'} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
              style={{ width: `${sidebarWidth}px` }}
              aria-label="Velclaw workspace navigation"
            >
            ...
            <div
              className={`hidden lg:block fixed inset-y-0 cursor-col-resize group z-50 ${isSidebarOpen ? 'w-1 opacity-100' : 'w-0 opacity-0'}`}
            ...
            <main
              className={`flex-1 min-w-0 overflow-auto flex flex-col ${isResizing || !hasMounted ? '' : 'transition-[margin] duration-200 ease-out'}`}
            ...
            >
              <div className="sticky top-0 z-20 flex h-12 shrink-0 items-center gap-2 border-b bg-background/95 px-3 backdrop-blur supports-[backdrop-filter]:bg-background/80">
                <Button
                  ...
                  aria-label={isSidebarOpen ? 'Close workspace sidebar' : 'Open workspace sidebar'}
                >
