...
const FEATURED_RFCS: RfcDoc[] = [
  {
    ...
    abstract:
      'The Hypertext Transfer Protocol (HTTP) is a stateless application-level protocol for distributed, collaborative, hypertext information systems. This document defines the HTTP semantics, independent of any transport or wire representation.',
...
`,
  },
  {
    ...
    abstract:
      'This document specifies version 1.3 of the Transport Layer Security (TLS) protocol. TLS 1.3 provides communications security over the Internet, preventing eavesdropping, tampering, and message forgery.',
...
`,
  },
  {
    ...
    date: 'October 2012',
    abstract:
      'The OAuth 2.0 authorization framework enables a third-party application to obtain limited access to an HTTP service, either on behalf of a resource owner by orchestrating an approval interaction or by allowing the application to obtain access on its own behalf.',
...
export const RfcViewer: React.FC = () => {
  ...
  return (
    <div className="flex-1 bg-black text-white font-mono text-xs flex flex-col md:flex-row h-full overflow-hidden border-t-2 border-neutral-800 select-text">
      ...
      <div
        className={`w-full md:w-80 bg-black border-r-2 border-neutral-800 flex-col shrink-0 ${
      ...
      <div
        className={`flex-1 flex-col bg-black overflow-hidden h-full ${
            ...
            <div className="bg-black p-4 sm:p-5 border-2 border-neutral-800 space-y-4">
              ...
              <div className="text-[11px] text-neutral-300 space-y-2 pt-2 border-t-2 border-neutral-900">
                <div className="font-extrabold text-white uppercase tracking-wider text-xs">
                  Thông tin bài viết RFC (Metadata):
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-neutral-400 font-bold">Trang chính thức:</span>{' '}
                    <a
                      href={`https://www.rfc-editor.org/info/rfc${selectedRfc.number}`}
                      target="_blank"
                      rel="noreferrer"
                      className="text-cyan-400 underline font-bold"
                    >
                      https://www.rfc-editor.org/info/rfc{selectedRfc.number}
                    </a>
                  </div>
                  <div>
                    <span className="text-neutral-400 font-bold">Tác giả:</span> {selectedRfc.authors}
                  </div>
                  <div>
                    <span className="text-neutral-400 font-bold">Ngày ban hành:</span> {selectedRfc.date}
                  </div>
                  <div>
                    <span className="text-neutral-400 font-bold">Trạng thái:</span> {selectedRfc.status}
                  </div>
                  {selectedRfc.obsoletes && (
                    <div className="col-span-1 sm:col-span-2">
                      <span className="text-neutral-400 font-bold">Thay thế bài viết:</span> {selectedRfc.obsoletes}
                    </div>
                  )}
                </div>
              </div>
