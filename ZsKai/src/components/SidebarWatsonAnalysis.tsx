...
export const SidebarWatsonAnalysis: React.FC<SidebarWatsonAnalysisProps> = ({
  ...
  useEffect(() => {
    ...
    setHistory((prev) => {
      ...
      return [
        ...prev,
        { time: timeStr, timestamp: now, latency: metrics.queryLatencyMs, efficiency: metrics.nodeEfficiency },
      ]
    })
  ...
  return (
    <aside className="w-full lg:w-72 bg-black border-l-2 border-neutral-800 flex flex-col h-full select-none overflow-y-auto text-white font-mono">
      <div className="p-5 space-y-6 flex-1">
          ...
          <div className="bg-black rounded-none p-4 border-2 border-neutral-700 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t-2 border-neutral-800 text-[10px]">
              ...
              <div className="bg-black p-2 rounded-none border border-neutral-700">
                ...
                <div
                  className={`font-bold text-xs font-mono ${metrics.anomaliesDetected > 0 ? 'text-amber-300' : 'text-cyan-300'}`}
                >
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1 bg-black p-1 rounded-none border-2 border-neutral-700">
              ...
              >
                <option value="15m" className="bg-black text-white">
                  Last 15m
                </option>
                <option value="1h" className="bg-black text-white">
                  Last 1h
                </option>
                <option value="24h" className="bg-black text-white">
                  Last 24h
                </option>
        ...
        <div className="bg-black p-4 rounded-none border-2 border-neutral-700 shadow-lg">
          <div className="text-xs font-bold text-cyan-300 mb-2 flex items-center justify-between">
            ...
            <span className="text-[9px] bg-neutral-900 text-cyan-300 px-1.5 py-0.5 border border-neutral-700 rounded-none font-mono font-bold">
              WATSON ML
            </span>
          </div>
          <p className="text-[11px] leading-relaxed text-neutral-200 font-medium">
            {metrics.anomaliesDetected > 0 ? (
              <>
                Detected <span className="text-amber-300 font-bold">{metrics.anomaliesDetected} duplicate entries</span>{' '}
                in <code className="text-cyan-300">cloudant_v3_log</code>.
              </>
            ) : (
              <>
          ...
          <button
            onClick={() =>
              onTriggerQuickCommand(
                metrics.anomaliesDetected > 0 ? 'watson prune --duplicates' : 'watson optimize --dry-run',
              )
            }
      ...
      <div className="p-4 border-t-2 border-neutral-800 bg-black">
        ...
        <div className="mt-1 text-[10px] text-neutral-400 font-mono truncate select-all">SHA-256: {encryptedHash}</div>
      </div>
    </aside>
  )
}
