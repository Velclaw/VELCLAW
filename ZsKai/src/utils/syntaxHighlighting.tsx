  ...
  return tokens.map((token, idx) => {
    ...
    if (isFirstWord) {
      ...
      if (
        ...
        return (
          <span key={idx} className={theme.mainCmd}>
            {token}
          </span>
        )
      }
      return (
        <span key={idx} className={theme.fallbackCmd}>
          {token}
        </span>
      )
    }
    ...
    if (token.startsWith('-')) {
      return (
        <span key={idx} className={theme.flag}>
          {token}
        </span>
      )
    }
    ...
    if ((token.startsWith('"') && token.endsWith('"')) || (token.startsWith("'") && token.endsWith("'"))) {
      return (
        <span key={idx} className={theme.stringVal}>
          {token}
        </span>
      )
    }
    ...
    if (token.includes('=') || token.includes('/')) {
      return (
        <span key={idx} className={theme.keyValue}>
          {token}
        </span>
      )
    }
    ...
    if (
      ...
      return (
        <span key={idx} className={theme.subCmd}>
          {token}
        </span>
      )
    }
    ...
    if (/^\d+(\.\d+)?$/.test(token)) {
      return (
        <span key={idx} className={theme.numberVal}>
          {token}
        </span>
      )
    }
    ...
    return (
      <span key={idx} className={theme.defaultText}>
        {token}
      </span>
    )
  })
    ...
    <pre className="text-xs font-mono leading-relaxed bg-[#111622] p-3 rounded-lg border border-slate-800/80 overflow-x-auto my-2 text-slate-200">
      {lines.map((line, lineIdx) => (
        <div key={lineIdx} className="table-row">
          ...
          <span className="table-cell pl-2">{highlightCodeLine(line, language)}</span>
        </div>
      ))}
    </pre>
  )
...
function highlightCodeLine(line: string, language: string): React.ReactNode {
  if (language === 'json') {
    ...
    if (match) {
      ...
      return (
        <span>
          {match[1]}
          <span className="text-blue-300 font-semibold">{match[2]}</span>:{highlightValue(rest)}
        </span>
      )
    }
  }
  ...
  const parts = line.split(
    /(\b(?:const|let|var|function|return|import|from|export|default|async|await|if|else|SELECT|FROM|WHERE|INSERT|UPDATE|DELETE|def)\b|"[^"]*"|'[^']*'|`[^`]*`|\d+)/g,
  )
