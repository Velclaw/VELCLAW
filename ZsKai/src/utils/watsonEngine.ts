...
export const SUGGESTIONS_DICTIONARY: CommandSuggestion[] = [
  {
    ...
    category: 'Watson AI',
    description: 'Run full Watson ML data pipeline analysis',
  },
  {
    command: 'watson optimize --dry-run',
    category: 'Watson AI',
    description: 'Simulate index & query performance optimization',
  },
  {
    command: 'watson sync-devices --target="ios-mobile-v2"',
    category: 'Watson AI',
    description: 'Sync database state with connected cross-platform devices',
  },
  {
    command: 'watson prune --duplicates',
    category: 'Watson AI',
    description: 'Clean up duplicate log records and anomalies',
  },
  {
    command: 'watson status',
    category: 'Watson AI',
    description: 'Check Watson ML pipeline status and confidence score',
  },
  {
    command: 'watson ai-query "Phân tích hiệu năng hệ thống Cloudant"',
    category: 'Watson AI',
    description: 'Query Gemini/Watson AI for system analysis',
  },
  {
    command: 'cloudant sync --force',
    category: 'Cloudant',
    description: 'Force full replication cycle with Cloudant cluster',
  },
  { command: 'cloudant status', category: 'Cloudant', description: 'Check health and latency of Cloudant DB nodes' },
  {
    command: 'git status',
    category: 'Git / DevOps',
    description: 'View current branch status and pending uncommitted changes',
  },
...
- **\`clear\`**: Xóa sạch màn hình terminal`,
  ...
  if (lower.startsWith('watson set-confidence') || lower.startsWith('watson confidence')) {
    ...
    if (!isNaN(val)) {
      ...
      const label =
        boundedScore >= 0.9
          ? 'High Reliability (Optimal)'
          : boundedScore >= 0.7
            ? 'Moderate Caution'
            : 'Low / Degraded Reliability'
...
async function callGeminiAPI(prompt: string): Promise<string> {
  ...
  const apiKey =
    metaEnv.VITE_GEMINI_API_KEY || (typeof process !== 'undefined' ? process.env?.GEMINI_API_KEY : undefined)
