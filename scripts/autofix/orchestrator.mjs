...
const baseUrl = (process.env.LLM_BASE_URL || 'https://velclaw-ai-playground.lovable.app/api/public/v1').replace(/\/$/, '')
...
const response = await fetch(`${baseUrl}/chat/completions`, {
  ...
  body: JSON.stringify({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are Velclaw AutoFix. Return only valid JSON matching the requested schema. Never expose secrets. Make minimal, production-safe code changes.',
      },
...
if (!response.ok) throw new Error(`Velclaw AI API failed: ${response.status} ${await response.text()}`)
const body = await response.json()
const content = body?.choices?.[0]?.message?.content
if (typeof content !== 'string' || !content.trim()) throw new Error('Velclaw AI returned no message content.')
