...
const DOCS_SITE_SLUGS = new Set(['bat-dau', 'cai-dat', 'cau-hinh', 'trien-khai'])
...
export default async function DocsArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (DOCS_SITE_SLUGS.has(slug)) return <DocsSite />
  ...
  return (
    <main className="min-h-screen bg-black px-6 py-12 text-zinc-200">
      <article className="mx-auto max-w-4xl">
        <a href="/docs" className="font-mono text-xs text-violet-400">
          ← /docs
        </a>
        <div className="mt-8 space-y-4 font-mono text-sm leading-7">
          {source.split('\n').map((line, index) => {
            if (line.startsWith('# '))
              return (
                <h1 key={index} className="text-3xl font-semibold text-white">
                  {line.slice(2)}
                </h1>
              )
            if (line.startsWith('## '))
              return (
                <h2 key={index} className="mt-8 text-xl font-semibold text-white">
                  {line.slice(3)}
                </h2>
              )
            if (line.startsWith('### '))
