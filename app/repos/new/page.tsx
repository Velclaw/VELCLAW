              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                {REPO_TEMPLATES.map((template) => (
                  <button
                    ...
                    disabled={isCreatingRepo}
                    className={`relative group transition-all ${isCreatingRepo ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  >
                    <div
                      className={`md:hidden flex items-center gap-3 p-3 rounded-md border-2 transition-all ${selectedTemplate === template.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`}
                    >
                    ...
                    <div className="hidden md:block">
                      <div
                        className={`aspect-video rounded-md overflow-hidden bg-muted mb-2 border-2 transition-all flex items-center justify-center text-foreground ${selectedTemplate === template.id ? 'border-primary' : 'border-border hover:border-primary/50'}`}
                      >
                        {template.id === 'empty' ? (
