      ...
      <main className="min-h-screen bg-[#050608] text-white flex items-center justify-center px-5">
        <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/[.045] p-8 shadow-2xl backdrop-blur-xl">
          ...
          <p className="mt-3 text-sm leading-6 text-white/60">
            Velclaw will securely authenticate your account before asking for permission.
          </p>
          <Link
            href={loginUrl}
            className="mt-7 flex h-12 items-center justify-center rounded-xl bg-white text-sm font-semibold text-black transition hover:bg-white/90"
          >
            Continue with Velclaw
          </Link>
    ...
    <main className="min-h-screen bg-[#050608] text-white flex items-center justify-center px-5 py-10">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/[.045] p-8 shadow-2xl backdrop-blur-xl">
        ...
        <div className="mt-8 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-white/40">REQUESTING ACCESS</p>
        ...
        <div className="mt-5 space-y-2">
          {scopes.map((scope) => (
            <div
              key={scope}
              className="flex items-center gap-3 rounded-xl border border-white/8 bg-black/20 px-4 py-3 text-sm"
            >
              <span className="h-2 w-2 rounded-full bg-white" />
              {scope === 'openid'
                ? 'Verify your Velclaw identity'
                : scope === 'profile'
        ...
        <form action="/api/oauth/authorize" method="POST" className="mt-7 space-y-3">
          ...
          <button
            name="decision"
            value="allow"
            className="h-12 w-full rounded-xl bg-white text-sm font-semibold text-black hover:bg-white/90"
          >
            Allow access
          </button>
          <button
            name="decision"
            value="deny"
            className="h-12 w-full rounded-xl border border-white/10 text-sm font-medium text-white/70 hover:bg-white/5"
          >
            Cancel
          </button>
        </form>
...
function Brand() {
  return (
    <div className="flex items-center gap-3">
      <div className="h-9 w-9 rounded-xl bg-white text-black flex items-center justify-center font-black">V</div>
      <div>
        <div className="font-semibold tracking-tight">VELCLAW</div>
        <div className="text-[10px] tracking-[.22em] text-white/35">IDENTITY</div>
      </div>
    </div>
  )
}
function OAuthError({ message }: { message: string }) {
  return (
    <main className="min-h-screen bg-[#050608] text-white flex items-center justify-center px-5">
      <div className="w-full max-w-md rounded-3xl border border-red-400/20 bg-red-400/[.04] p-8">
        <Brand />
        <h1 className="mt-8 text-xl font-semibold">OAuth request rejected</h1>
        <p className="mt-3 text-sm leading-6 text-white/60">{message}</p>
        <Link
          href="/auth/signin"
          className="mt-7 flex h-12 items-center justify-center rounded-xl bg-white text-sm font-semibold text-black hover:bg-white/90"
        >
          Về trang đăng nhập Velclaw
        </Link>
      </div>
    </main>
  )
