...
export async function POST(request: NextRequest) {
  ...
  if (!/^[0-9a-f]{40}$/i.test(commitSha))
    return NextResponse.json({ error: 'A 40-character commit SHA is required' }, { status: 400 })
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Release provider unavailable' },
      { status: 503 },
    )
  }
}
