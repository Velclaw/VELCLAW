...
const REPO_PATTERN =
  /^https:\/\/github\.com\/[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})\/[A-Za-z0-9][A-Za-z0-9._-]{0,99}(?:\.git)?$/i
...
export async function POST(request: NextRequest) {
  ...
  if (!REPO_PATTERN.test(repoUrl))
    return NextResponse.json({ error: 'Only canonical HTTPS GitHub repository URLs are supported' }, { status: 400 })
  ...
  if (customDomain && !isVelclawHostname(customDomain))
  ...
  if (env) {
    ...
    if (
      ...
      entries.some(
        ([key, value]) =>
          !ENV_KEY_PATTERN.test(key) || typeof value !== 'string' || value.length > 8192 || /[\r\n]/.test(value),
      )
    )
  ...
  try {
    const created = await createHostingDeployment({
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Deployment provider unavailable' },
      { status: 503 },
    )
  }
}
