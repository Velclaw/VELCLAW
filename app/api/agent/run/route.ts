...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const taskId = typeof body.taskId === 'string' && body.taskId ? body.taskId : generateId(12)
    ...
    const run = async () => {
      ...
      try {
        ...
        await db.update(tasks).set({
          ...
          branchName: sandboxResult.branchName || branchName,
          updatedAt: new Date(),
        ...
        const pushResult = await pushChangesToBranch(
          sandbox,
          sandboxResult.branchName || branchName,
          createFallbackCommitMessage(prompt),
          logger,
        )
    ...
    return json({
      ...
      capabilities: { planner: true, filesystem: true, terminal: true, runtime: true, tests: true, review: true, preview: true, git: true },
    }, 202)
  } catch (error) {
