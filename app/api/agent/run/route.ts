...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await db.insert(tasks).values({ id: taskId, userId, prompt, title: prompt.slice(0, 60), repoUrl, selectedAgent: agent, selectedModel: model, installDependencies: true, maxDuration, keepAlive: true, enableBrowser: Boolean(body.enableBrowser), status: 'pending', progress: 0, logs: [], branchName })
    ...
    const run = async () => {
      ...
      try {
        ...
        const apiKeys = { OPENAI_API_KEY: process.env.OPENAI_API_KEY, GEMINI_API_KEY: process.env.GEMINI_API_KEY, CURSOR_API_KEY: process.env.CURSOR_API_KEY, ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY, AI_GATEWAY_API_KEY: process.env.AI_GATEWAY_API_KEY }
        const port = await detectPortFromRepo(repoUrl, githubToken)
        const sandboxResult = await createSandbox({ taskId, repoUrl, githubToken, gitAuthorName: process.env.GITHUB_USER_NAME || 'Velclaw Agent', gitAuthorEmail: process.env.GITHUB_USER_EMAIL || 'agent@velclaw.local', apiKeys, timeout: `${maxDuration}m`, ports: [port], runtime: 'node22', resources: { vcpus: 4 }, taskPrompt: prompt, selectedAgent: agent, selectedModel: model, installDependencies: true, keepAlive: true, enableBrowser: Boolean(body.enableBrowser), preDeterminedBranchName: branchName, onProgress: (progress, message) => logger.updateProgress(progress, message) }, logger)
      ...
      } finally { if (sandbox && process.env.AGENT_SHUTDOWN_AFTER_BUILD === 'true') { try { await sandbox.stop() } catch {} } }
    }
    after(run)
  ...
  } catch (error) { return json({ error: error instanceof Error ? error.message : 'Failed to start agent run' }, 500) }
}
...
export async function PATCH(req: NextRequest) {
  ...
  const [task] = await db.select().from(tasks).where(and(eq(tasks.id, taskId), eq(tasks.userId, process.env.VELCLAW_AGENT_USER_ID || ''), isNull(tasks.deletedAt))).limit(1)
  if (!task) return json({ error: 'Task not found' }, 404)
  if (task.status !== 'processing' && task.status !== 'pending') return json({ error: 'Task can only be stopped while running' }, 400)
  const logger = createTaskLogger(taskId)
  await logger.info('Stop request received from Velclaw Builder')
  const [updated] = await db.update(tasks).set({ status: 'stopped', error: 'Task was stopped by user', completedAt: new Date(), updatedAt: new Date() }).where(eq(tasks.id, taskId)).returning()
  try { await killSandbox(taskId) } catch (error) { console.error('Failed to kill sandbox:', error) }
