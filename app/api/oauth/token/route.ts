...
import { createAccessToken, findOAuthClient, verifyAuthorizationCode } from '@/lib/velclaw/oauth'
...
export async function POST(req: NextRequest) {
  ...
  const clientId = String(body.client_id || basicCredentials(req)?.clientId || '')
  const clientSecret = String(body.client_secret || basicCredentials(req)?.clientSecret || '')
  ...
  try {
    ...
    if (payload.client_id !== clientId || payload.redirect_uri !== redirectUri || typeof payload.user_id !== 'string') {
