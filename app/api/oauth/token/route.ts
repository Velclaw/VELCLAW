...
function basicCredentials(req: NextRequest) {
  ...
  try {
    ...
    return index < 0 ? null : { clientId: decoded.slice(0, index), clientSecret: decoded.slice(index + 1) }
  } catch {
    return null
  }
}
...
export async function POST(req: NextRequest) {
  ...
  const clientId = String(body.client_id || basic?.clientId || ''),
    clientSecret = String(body.client_secret || basic?.clientSecret || '')
    ...
    if (typeof payload.code_challenge === 'string') {
      if (
        !verifyPkce(
          ...
          String(payload.code_challenge_method || ''),
        )
      )
