...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    domains:
      await sql`SELECT hostname, deployment_id as "deploymentId", created_at as "createdAt" FROM velclaw_deploy_domains ORDER BY created_at DESC`,
  })
}
...
export async function POST(request: Request) {
  ...
  if (!DOMAIN.test(hostname))
    return NextResponse.json({ error: 'Only *.velclaw.cfd hostnames are supported' }, { status: 400 })
  await ensureTable()
  ...
  if (deployment.status !== 'ready')
    return NextResponse.json({ error: 'Only ready deployments can receive domains' }, { status: 409 })
