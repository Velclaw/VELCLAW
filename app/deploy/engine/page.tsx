    ...
    <main className="flex-1 overflow-auto p-4 md:p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header>
          ...
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Self-hosted deployment control plane. GitHub is the source of truth; the worker builds releases without relying on Vercel.</p>
        </header>
        ...
        <form onSubmit={submit} className="grid gap-3 border border-border bg-card p-4 md:grid-cols-4">
          <label className="space-y-1 text-xs"><span>Project</span><input className="w-full border border-border bg-background px-3 py-2" value={projectName} onChange={(e) => setProjectName(e.target.value)} /></label>
          <label className="space-y-1 text-xs md:col-span-2"><span>GitHub repository</span><input className="w-full border border-border bg-background px-3 py-2" value={repoUrl} onChange={(e) => setRepoUrl(e.target.value)} /></label>
          <label className="space-y-1 text-xs"><span>Branch</span><input className="w-full border border-border bg-background px-3 py-2" value={branch} onChange={(e) => setBranch(e.target.value)} /></label>
          <div className="flex items-center gap-3 md:col-span-4"><button disabled={busy} className="border border-violet-400/60 bg-violet-500/10 px-4 py-2 text-sm disabled:opacity-50">{busy ? 'Queueing…' : 'Deploy'}</button>{error && <span className="text-xs text-red-300">{error}</span>}</div>
        </form>

        <section className="space-y-3">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Deployments</h2><p className="text-xs text-muted-foreground">Queue → isolated build worker → verified build artifact → runtime publication.</p></div><button onClick={() => void refresh()} className="border border-border px-3 py-2 text-xs">Refresh</button></div>
