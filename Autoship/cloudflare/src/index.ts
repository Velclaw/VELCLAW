...
const projectView = (p: Row) => ({
  ...
  lastDeployStatus: p.last_deploy_status,
  totalBuilds: p.total_builds,
})
const runView = (r: Row) => ({
  id: r.id,
  ...
  commitMessage: r.commit_message || r.message || '',
  branch: r.branch,
  triggeredBy: r.triggered_by || 'autoship',
...
function requestOrigin(request: Request, env: Env) {
  const configured = (env.AUTOSHIP_ALLOWED_ORIGINS || '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean)
...
function authorized(request: Request, env: Env) {
  return !env.AUTOSHIP_API_TOKEN || request.headers.get('authorization') === `Bearer ${env.AUTOSHIP_API_TOKEN}`
}
...
async function route(request: Request, env: Env) {
  ...
  if (url.pathname === '/api/health')
    return json({ ok: true, service: 'autoship-control-plane', runtime: 'cloudflare-workers' }, 200, origin)
  if (url.pathname === '/api/ready') {
    try {
      await env.DB.prepare('SELECT 1').first()
      return json({ ok: true, database: 'd1' }, 200, origin)
    } catch (error) {
      return json({ ok: false, error: String(error) }, 503, origin)
    }
  }
  ...
  if (url.pathname === '/api/deployments/callback' && request.method === 'POST') {
    ...
    if (!runId || !['queued', 'running', 'success', 'failed', 'cancelled'].includes(status))
      return json({ error: 'Invalid callback' }, 400, origin)
    await env.DB.prepare('UPDATE runs SET status=?, updated_at=?, message=? WHERE id=?')
      .bind(status, new Date().toISOString(), body.message ? String(body.message) : null, runId)
      .run()
  ...
  if (url.pathname === '/api/projects' && request.method === 'GET') {
    ...
    return json({ projects: rows.results.map(projectView) }, 200, origin)
  }
  if (url.pathname === '/api/projects/upsert' && request.method === 'POST') {
    ...
    const now = new Date().toISOString(),
      existing = await env.DB.prepare('SELECT * FROM projects WHERE repo_url=?').bind(repoUrl).first<Row>()
    const project = {
    ...
    await env.DB.prepare(
      `INSERT INTO projects (id,repo_url,name,branch,target,framework,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(repo_url) DO UPDATE SET name=excluded.name,branch=excluded.branch,target=excluded.target,framework=excluded.framework,updated_at=excluded.updated_at`,
    )
      .bind(
        project.id,
        project.repo_url,
  ...
  if (url.pathname === '/api/pipelines/trigger' && request.method === 'POST') {
    const body = (await request.json()) as Record<string, unknown>,
      ...
      project = await env.DB.prepare('SELECT * FROM projects WHERE id=?').bind(projectId).first<Row>()
    if (!project) return json({ error: 'Project not found' }, 404, origin)
    const runId = id('run'),
      now = new Date().toISOString(),
      commitHash = body.commitHash ? String(body.commitHash) : null,
      branch = String(body.branch || project.branch)
    const run = {
      id: runId,
      ...
      triggered_by: body.triggeredBy || 'autoship',
    }
    await env.DB.prepare(
      'INSERT INTO runs (id,project_id,status,commit_hash,branch,created_at,updated_at,message) VALUES (?,?,?,?,?,?,?,?)',
    )
      .bind(
        run.id,
        run.project_id,
      ...
      .run()
    try {
      await githubDispatch(env, {
        runId,
        projectId,
        repoUrl: project.repo_url,
        repo: repoSlug(project.repo_url),
        branch,
        commitHash,
        target: project.target,
      })
    } catch (error) {
      await env.DB.prepare("UPDATE runs SET status='failed',updated_at=?,message=? WHERE id=?")
        .bind(new Date().toISOString(), String(error), runId)
        .run()
      return json(
        {
          error: 'Unable to dispatch GitHub Actions',
          run: runView({ ...run, status: 'failed', message: String(error) }),
        },
        502,
        origin,
      )
  ...
  if (runMatch && request.method === 'GET') {
    const run = await env.DB.prepare('SELECT * FROM runs WHERE id=?').bind(runMatch[1]).first<Row>()
    return run ? json({ run: runView(run) }, 200, origin) : json({ error: 'Run not found' }, 404, origin)
  }
